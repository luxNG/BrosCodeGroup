...
namespace FurnitureCompany.Repository
{
    public class EmployeeRepository:IEmployeeRepository
    {
        ...
        {
            //đang bị lỗi tại đây
              Order orderDetail = furnitureCompanyContext.Orders.Where(x => x.OrderId == orderId).Include(x => x.OrderServices).ThenInclude(x => x.Service).Include(x => x.Customer).Include(x => x.WorkingStatus).Include(x=>x.Assigns).ThenInclude(x=>x.Employee).FirstOrDefault();
